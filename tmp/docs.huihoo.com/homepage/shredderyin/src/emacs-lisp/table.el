...
;; Revised:       Fri Nov 29 2002 23:02:44 (PST)
...
;;;
(eval-when-compile
  (require 'backquote)) ;; expecting Emacs 19.29 or later for concise backquote expression, see ONEWS
...
(when (locate-library "quail")
  (require 'quail)
  ;; some version of quail-start-conversion fail to clear quail-translating
  (defadvice quail-start-conversion (after quail-clear-quail-translating last activate compile)
...
non-nil."
  ...
  (unless table-disable-advising
    ...
    (table-advice-do-at-point-in-cache clipboard-yank
      ad-do-it
      (table--untabify (point-min) (point-max))
      (table--fill-region (point-min) (point-max))
      (setq table-inhibit-auto-fill-paragraph t))
  ...
  (let ((default-buffer-name (concat table-dest-buffer-name "." (symbol-name language))))
	...
	(table--generate-source-epilogue dest-buffer language col-list row-list))
	  ...
	  (save-selected-window
	    ...
	    (let ((mode
		   (if (memq language '(cals tei)) 'sgml-mode
...
(defun *table--cell-quoted-insert (arg)
  ...
  (interactive "*p")
  (let ((table-inhibit-advice t)
	(char (table--unibyte-char-to-multibyte (read-quoted-char))))
    (while (> arg 0)
